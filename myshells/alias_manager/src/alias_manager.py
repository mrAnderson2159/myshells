...
sys.path.append(os.path.expanduser("~/myshells/myshells/pyplugs"))
...
def list_aliases(
    source_file: Union[str, os.PathLike, PosixPath] = bash_profile(),
) -> list[str]:
    """
...
def backup_cleaner(source_file: Union[str, os.PathLike, PosixPath] = bash_profile()):
    try:
        ...
        backup_files = [
            ...
            if file.name.startswith(backup_name) and file.name.endswith(".backup")
        ]
        ...
        for backup_file in older_backup_files:
            try:
                ...
                print(
                    f"{c_yellow('Warning:')} Older backup {c_cyan(backup_file.name)} sent to trash"
                )
            except Exception as e:
                print(
                    f"{c_red('Error:')} sending {c_yellow(backup_file.name)} to trash: {e}"
                )
...
def update(
    lines: list[str], source_file: Union[str, os.PathLike, PosixPath] = bash_profile()
):
    """
...
def split_alias(alias_line: str) -> tuple:
    ...
    if not match:
        raise ValueError(
            f"Error: The alias line '{alias_line}' does not match the expected format."
        )
...
def newalias(alias_name: str, alias_body: tuple[str]):
    ...
    alias_cmd = build_alias(
        alias_name, alias_body_str
    )  # f"alias {alias_name}='{alias_body_str}'"
    existing_alias = None
    ...
    print(
        f"Alias {c_cyan(alias_name)} {'updated' if existing_alias else 'added'} {c_green('successfully')}."
    )
...
def mvalias(alias_name: str, body: str, name: str) -> None:
    ...
    if name:
        ...
        if click.confirm(
            ...
            print(
                f"Alias {c_cyan(alias_name)} -> {c_yellow(alias_data['body'])} has not been modified\n"
            )
    else:
        print(
            f"{c_yellow('No changes made.')} Please provide a new name or a new body for the alias."
        )
...
def rmalias(alias_names: tuple[str]):
    ...
    for alias_name in alias_names:
        ...
        if len(lines) != len(new_lines):
            if click.confirm(
                f"Are you sure you want to delete alias {c_yellow(alias_name)}?"
            ):
                print(
                    f"Alias {c_cyan(alias_name)} has been {c_green('successfully deleted')}."
                )
                lines = new_lines
                modified = True
            else:
