...
void execBuildPlus(char *fileName){
    char* compile = malloc((strlen(fileName)*3)+46);
    // c'era anche il flag -Werror che trasformava i warnings in errori D:
    sprintf(compile, "g++ -Wall -pedantic -std=c++17 -o %s %s.cpp; ./%s\n", fileName, fileName, fileName);
    system(compile);
    free(compile);
}
