...
def usage(indentation: int = 0) -> str:
    ...
    usage += "\t\t\t[exemple :16up16:32low32:40up returns a string in lowercase from beginning to 15th character,\n"
    usage += "\t\t\tin uppercase from 16th character to 31st and lower from 32nd to 40th]\n"
...
def process_args(argv: List[str]) -> str:
    ...
    if len(argv) < 2:
        raise A00(
            f"This program needs at least two argument: the sha type algorithm and the string to encrypt, {len(argv)} recieved {usage(1)}"
        )
    ...
    def process_compose() -> None:
        ...
        for arg in argv:
            ...
            if match:
                ...
                res = match_group(
                    map(lambda pattern: type_group(group(pattern)), res)
                )
                compose = list(res)
                break
...
def sha(
    type: str,
    string: str,
    case_function: Callable[[Callable], str] = str.lower,
    slicing: Tuple[int] = None,
    step: int = None,
    compose: List[tuple] = None,
) -> str:
